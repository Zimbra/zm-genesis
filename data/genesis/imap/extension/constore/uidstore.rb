...
current.description = "IMAP UID STORE BUG29487"
...
current.action = [  
  ...
  v(cb("fetch") do 
    mUID = m.fetch(1..1, "UID").first.attr['UID'] 
    m.uid_store(mUID, "-FLAGS", [:SEEN])  
  end) do |mcaller, data|
    mcaller.pass = data.class == Array && data.first.class == Net::IMAP::FetchData && data.first.attr.key?('MODSEQ')
  end, 
