...
current.description = "Share folders search test"
...
current.action = [    
                  ...
                  v(cb("Fetch From  Account Two Mailbox") {
                  ...
                  [
                   ...
                   ["BODY", "so#{1.chr}so"]
                    ...
                    SearchVerify.new(mimap, x, &IMAP::EmptyArray)
                  end, 
 
                  v(cb("Fetch From  Account Three Mailbox") {
                      mimap.logout
                      mimap.disconnect
                      mimap.object = Net::IMAP.new(origHost, *origHost.imap)
                      mimap.login(nameOneAccount.name, nameOneAccount.password)
