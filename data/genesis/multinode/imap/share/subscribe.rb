...
# 2006 Zimbra
#
# imap share folder subscribe test
...
current.description = "Share folders subscribe test"
                  ...
                  [mailbox].map do |x| 
                    ["/home/#{nameTwoAccount.name}", "/home/#{nameTwoAccount.name}"].map do |y|
                      [ SubscribeVerify.new(mimap, File.join(y, x)),
                        UnsubscribeVerify.new(mimap, File.join(y, x))]
                    end
                  end, 
                  ["INBOX", "Trash"].map do |x| 
                    ["/home/#{nameTwoAccount.name}", "/home/#{nameTwoAccount.name}"].map do |y|
                      [SubscribeVerify.new(mimap, File.join(y, x), &IMAP::SubscribeFailed),
                       ]
                    end
                  end, 
                  cb("Clean Up") do
