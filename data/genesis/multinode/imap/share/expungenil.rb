...
# 2006 Zimbra
#
# imap share folder expunge test
...
current.description = "Share folders check test"
...
current.action = [    
                  ...
                  cb("Set ACL Account Two") {               
                    ...
                    mimap2.append(mailbox,message.gsub(/REPLACEME/, nameOneAccount.name),[:Seen], Time.now)
                    mimap2.append(mailbox,message.gsub(/REPLACEME/, nameOneAccount.name),[:Seen], Time.now)
                    mimap2.append(mailbox,message.gsub(/REPLACEME/, nameOneAccount.name),[:Seen], Time.now)
                    mimap2.logout
                    mimap2.disconnect
                  ...
                  cb("Log into account two") do
                    mimap2.object = Net::IMAP.new(origHost, *origHost.imap)
                    mimap2.login(nameTwoAccount.name, nameTwoAccount.password)
                    mimap2.select(mailbox)
                    mimap2.store(2..2, "+FLAGS", [:Deleted])
                  end,
                  ExpungeVerify.new(mimap2, "%s"%mailbox, { 'RECENT' => [0]}), 
                  FetchVerify.new(mimap, (1..-1),'BODY[]', '.') do |mcaller, data|
