...
current.description = "Share folders examine test"
...
mailbox = "INBOX/share"
mailboxes = ["/home/#{nameTwoAccount.name}/%s"%mailbox, "/home/#{nameThreeAccount.name}/%s"%mailbox, "INBOX"]
badones = ["/home/#{nameTwoAccount.name}/INBOX", "/home/#{nameTwoAccount.name}/INBOX"]
...
current.action = [    
                  ...
                  mailboxes.map do |thisone|
                    ExamineVerify.new(mimap, thisone)  
                  end,  
                  badones.map do |thisone|
                    ExamineVerify.new(mimap, thisone, &IMAP::ExamineFailed)  
                  end, 
                  cb("Change Folder") do
                    mimap.select("/home/#{nameThreeAccount.name}/%s"%mailbox)
                  end,
                  mailboxes.map do |thisone|
                    ExamineVerify.new(mimap, thisone)  
                  end,
