...
current.description = "Dedup ID 26645"

name = 'dedup'+File.basename(__FILE__,'.rb')+Time.now.to_i.to_s
...
current.action = [  
                  ...
                  ZMProv.new('ma %s zimbraPrefMessageIdDedupingEnabled FALSE'%testAccount.name),
                  ...
                  ZMProv.new('ma %s zimbraPrefMessageIdDedupingEnabled TRUE'%testAccount.name),
                  SendMail.new(testAccount.name, message),
                  WaitQueue.new,
                  v(cb("Check for number of messages") do
                      m.object = Net::IMAP.new(Model::TARGETHOST, * Model::TARGETHOST.imap)
