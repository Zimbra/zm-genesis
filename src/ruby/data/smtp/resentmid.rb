...
current.description = "Resnd-Msg ID 36297"

name = 'resend'+File.basename(__FILE__,'.rb')+Time.now.to_i.to_s
...
resendheader = <<EOF2
Resent-From: whatever@zimbra.com
Resent-Date: Tue, 2 Aug 2005 09:59:20 +0200
Resent-Message-ID: <429BB1F8-71FA-4AE1-95D6-F6C589DDEC59@testme.com>
EOF2
...
current.action = [  
                  ...
                  SendMail.new(testAccount.name, resendheader+message),
                  WaitQueue.new,
                  v(cb("Check for number of messages") do
                      m.object = Net::IMAP.new(Model::TARGETHOST, * Model::TARGETHOST.imap)
                      m.login(testAccount.name, testAccount.password)
                      m.select('INBOX')
                      m.responses
                    end) do |mcaller, data|
                    mcaller.pass =  (data['EXISTS'].first == 2)
                  end,
                  cb("Clean Up") do
                    m.logout
                    m.disconnect
