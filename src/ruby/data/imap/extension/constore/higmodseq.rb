...
current.description = "IMAP HIGHMODESEQUECE BUG18247"
...
Received: from lab-loadgen02.zimbra.com ([4.8.240.39]) by zimbra.com with Microsoft SMTPSVC(6.0.3790.211);
...
current.action = [  
  ...
  p(m.method('create'),"INBOX/FETCHBASICTWO"),
  cb("Create 2 messages using append") {       
    1.upto(2) { |i|
      m.append("INBOX/FETCHBASICTWO",message.gsub(/REPLACEME/,i.to_s),[:Seen], Time.now) 
    }
    "Done"
  },
  p(m.method('select'),"INBOX/FETCHBASICTWO"),
   
  v(ZMailAdmin.new('-m', testAccount.name, 's', '-t', 'conversation', '\'in "INBOX/FETCHBASICTWO"\'')) do |mcaller, data|
      result = data[1].scan(/(\d+)\s+con/m).flatten
      # Now move message to another folder
      result.each do |x|
        ZMailAdmin.new('-m', testAccount.name, 'mc', x, 'INBOX/FETCHBASIC').run
      end
      mcaller.pass = true 
  end,
  ...
  v(cb("modsequence check") do
    m.fetch((1..-1), ['MODSEQ']) 
  end 
  ) do |mcaller, data|
    # Check to see if all modseq is lower than highestmodeseq
    check = data.map {|x| x.attr['MODSEQ'].to_i}.all? {|y| y <= m.responses['HIGHESTMODSEQ'].first.to_i} 
    mcaller.pass = check
  end, 
