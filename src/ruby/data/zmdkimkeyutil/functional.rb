...
current.description = "Functional test zmdkimkeyutil"
...
current.action = [
  ...
  v(ZMDkimkeyutil.new("-a", "-d", Model::TARGETHOST, run_on_mta)) do |mcaller,data|
  ...
  Action::SendMail.new(testAccount.name, message, testAccount2.name),
  ...
  v(cb("Check message headers") do
    ...
    mcaller.pass = !data[0]["BODY[HEADER.FIELDS (DKIM-SIGNATURE)]"].chomp.empty? &&
                   ...
                   data[0]["BODY[HEADER.FIELDS (DKIM-SIGNATURE)]"].match(/s=#{Regexp.quote(uuid)};/)
  end,
  ...
  v(ZMDkimkeyutil.new("-q", "-d", Model::TARGETHOST.to_s, run_on_mta)) do |mcaller,data|
      mcaller.pass = data[0] = 0 &&
                     ...
                     data[1].match(/DKIM Public signature:\n#{Regexp.quote(uuid)}\._domainkey.*IN.*TXT.*v=DKIM1; k=rsa;.*p=.*;.*DKIM key #{Regexp.quote(uuid)} for #{Regexp.quote(Model::TARGETHOST)}/m)
  end,
  ...
  v(ZMDkimkeyutil.new("-u", "-d", Model::TARGETHOST.to_s, run_on_mta)) do |mcaller,data|
      mcaller.pass = data[0] = 0 &&
                     ...
                     data[1].match(/^(.*)\._domainkey/)[1] != uuid &&
                     data[1].match(/DKIM key (.*) for/)[1] != uuid
      if mcaller.pass
        uuid = data[1].match(/with selector (.*)\n/)[1]
      end
  end,
  
  # get new DKIM info for domain
  v(ZMDkimkeyutil.new("-q", "-d", Model::TARGETHOST.to_s, run_on_mta)) do |mcaller,data|
      mcaller.pass = data[0] = 0 &&
                     data[1].match("DKIM Domain:\n#{Model::TARGETHOST}") &&
                     data[1].match("DKIM Identity:\n#{Model::TARGETHOST}") &&
