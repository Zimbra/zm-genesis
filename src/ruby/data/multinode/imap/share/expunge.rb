...
current.action = [    
                  ...
                  ExpungeVerify.new(mimap, "/home/#{nameTwoAccount.name}/%s"%mailbox, { 'RECENT' => [1, 0]}), 
                  cb("Select Remote Share") do
                    mimap.select("/home/#{nameThreeAccount.name}/%s"%mailbox)
                  end,
                  ExpungeVerify.new(mimap, "/home/#{nameThreeAccount.name}/%s"%mailbox, { 'RECENT' => [1, 0]}), 
                  cb("Clean Up") do
                    mimap.logout
                    mimap.disconnect
                  end
