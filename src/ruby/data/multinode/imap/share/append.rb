...
current.description = "Share folders append test"
...
current.action = [    
                  ...
                  v(cb("Append to account two folder") {
                    mimap.object = Net::IMAP.new(origHost, *origHost.imap)
                    mimap.login(nameOneAccount.name, nameOneAccount.password)
                    result = []
                    result.push mimap.append("/home/#{nameTwoAccount.name}/%s"%mailbox, message.gsub(/REPLACEME/, nameOneAccount.name), ['JUNK'], Time.at(945702800))
                    result.push mimap.select("/home/#{nameTwoAccount.name}/%s"%mailbox)
                    result
                  }) do |mcaller, data|
                    mcaller.pass = data.all? {|x| x.class == Net::IMAP::TaggedResponse } && 
                      data.all? {|x| x['name'] == 'OK' } rescue false
                  end,
                 AppendVerify.new(1, [:Recent], Time.at(945702800), mimap),
                  v(cb("Append to account three folder") {        
                      result = []
                     Timeout::timeout(10) {
                        result.push mimap.append("/home/#{nameThreeAccount.name}/%s"%mailbox, message.gsub(/REPLACEME/, nameOneAccount.name), ['JUNK'], Time.at(945702800))
                        result.push mimap.select("/home/#{nameThreeAccount.name}/%s"%mailbox)
                      }
                      result
                  }) do |mcaller, data|
                    mcaller.pass = data.all? {|x| x.class == Net::IMAP::TaggedResponse } && 
