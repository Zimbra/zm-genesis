...
current.description = "Share folders select test"
...
current.action = [    
                  ...
                  [mailbox].map do |x| 
                    ["/home/#{nameTwoAccount.name}", "/home/#{nameTwoAccount.name}"].map do |y|
                      SelectVerify.new(mimap, File.join(y, x))
                    end
                  end, 
                  ["INBOX", "Trash"].map do |x| 
                    ["/home/#{nameTwoAccount.name}", "/home/#{nameTwoAccount.name}"].map do |y|
                      SelectVerify.new(mimap, File.join(y, x),  &IMAP::SelectFailed)
                    end
                  end, 
                  cb("Clean Up") do
                    mimap.logout
